...
Causal inference is performed with [Convergent Cross Mapping](https://en.wikipedia.org/wiki/Convergent_cross_mapping) ([CCM](https://science.sciencemag.org/content/338/6106/496)) ensuring the added observable is part of the same dynamical system as the target time series. The embedding dimension needed for CCM is automatically determined if parameter `E=0`, the default. Otherwise the specifed value of `E` is used. To account for unobserved variables time delay vectors of the top observables can be added.
